...
- **The read API and MCP server** (`Analysis/`, see [ai-access.md](ai-access.md)), which are invariant and UTC throughout and take neither a culture nor a zone. `UseRequestLocalization` sits in front of them, so `CurrentCulture` *is* set from `Accept-Language` on those requests — every number they format therefore names `InvariantCulture` explicitly, and every value they parse goes through `InputValues`. Their callers are programs: a duration that renders as `1,5s` to one client and `1.5s` to another is a bug however politely localized. Timestamps go out as ISO 8601 UTC in the form the API's own time filters accept. Don't wire `BrowserTimeZone` or a zone parameter into it — a zone could only come from the host, which in a container means nothing, or from a caller who already knows it and can convert without help, and zone rules change besides.
