...
`TracesRepository` holds a `Dictionary<string, TraceItem>` keyed by base64 trace id, plus a `ConcurrentDictionary<string, SpanRepository>` indexing every span by *name* — that second index is what powers the by-span-type views and their duration statistics.
...
`Controllers/TraceFilesController.cs` serves the same format: `GET /api/trace-files?traceId=…` downloads one trace, `POST` accepts uploads back in and replays the spans through `GetOrAddTrace`/`AddSpans`. Trace ids are base64 and can contain `/`, which is why the id is a query parameter rather than a route segment.
