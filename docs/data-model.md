...
Every reader and writer of the format goes through `TraceSerializableData.SerializerOptions`. Attributes are `Dictionary<string, object?>`, and System.Text.Json's stock handling of `object` returns a `JsonElement` for every value — so without the `AttributeValueJsonConverter` those options carry, a trace loaded from a file holds different CLR types than the same trace received over OTLP. That difference is invisible to anything that only calls `ToString`, which is most of the UI, and therefore stays broken quietly: it is what used to blank the Home page's custom attribute columns for uploaded traces. Use the options rather than a bare `JsonSerializer` call.
